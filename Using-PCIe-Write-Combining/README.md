...
Suppose instead of DDR memory there was a piece of hardware located at AppPF BAR4 that required individual writes to control particular functions. With WC instead of 16 individual writes, the hardware would only see a single access. Care must be taken when placing logic other than memory such as FIFOs in a WC region, because the order of writes or number of writes does not match the application writes.
