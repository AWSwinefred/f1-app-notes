...
When the BIU is stored is to strictly defined. Depending on the CPU version, it only contains a small number of these buffers (<=6). If another process accesses a different WC region, then the partially filled buffer may be flushed to make room. If an application must make sure that all writes were flushed from the BIU, then it is the software's responsiblity to use a x86 SFENCE instruction or similar mechanism to force the writes.
...
A simular issue exists if a FIFO is mapped into a WC region. The FIFO should decode across a 64 byte aligned address to prevent data written to the same address from being "eaten" by the BIU buffer, and the software must "spray" the data across the 64 byte buffer. For example, six byte writes to address 0 of BAR4 will result in a byte single write to address 0 when the buffer is stored. To ensure all six bytes are written to the FIFO, the software must write addresses 0 to 5.
